                                   ...
                                   local_planner_client_("local_planner_node_action", true)
{
  ...
  ROS_INFO("Global planer server wait for start!");
  global_planner_client_.waitForServer();
  ROS_INFO("Global planer server start!");
  ROS_INFO("Local planer server wait for start!");
...
ChassisExecutor::ExcutionMode ChassisExecutor::GetExecutionMode(){
...
void ChassisExecutor::GlobalPlannerFeedbackCallback(const roborts_msgs::GlobalPlannerFeedbackConstPtr& global_planner_feedback){
  if (!global_planner_feedback->path.poses.empty()) {
    ...
    local_planner_goal_.toward_goal = toward_goal_;
    local_planner_client_.sendGoal(local_planner_goal_);
    path_len_ = 0;
    for(unsigned int i = 0 ; i< global_planner_feedback->path.poses.size()-1 ; i++){
        float dx = global_planner_feedback->path.poses[i+1].pose.position.x - global_planner_feedback->path.poses[i].pose.position.x;
        float dy = global_planner_feedback->path.poses[i+1].pose.position.y - global_planner_feedback->path.poses[i].pose.position.y;
        path_len_ += std::sqrt(dx*dx + dy*dy);
    }
  }
}

void ChassisExecutor::SetTowardGoal(geometry_msgs::PoseStamped toward_goal){
    toward_goal_.pose = toward_goal.pose;
}

double ChassisExecutor::GetPathLen(){
    return path_len_;
}
