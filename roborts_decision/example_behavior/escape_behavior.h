  ...
  void Run() {
   ...
   //  escape to the new position---------------------------------------------------------------------------------------------
    if (executor_state != BehaviorState::RUNNING 
         || blackboard_->GetDistance(blackboard_->info.my_goal , my_pose) <= blackboard_->threshold.near_dist) {
        start = false;       
    }
    
    if ( (blackboard_->IsEnemyDetected() || blackboard_->GuardEnemyDetected()) && !start){    
        ...
        if (!blackboard_->GetCostMap2D()->World2Map(enemy.pose.position.x,
...
//          costmap_2d_->GetCost(goal_cell_x, goal_cell_y);
          ...
          // 条件满足 则跳出循环
          if (obstacle_count > obstacle_count_) { 
            break;
          }
        }
        ...
        Eigen::Vector2d pose_to_enemy(enemy.pose.position.x - escape_goal.pose.position.x,
                                      enemy.pose.position.y - escape_goal.pose.position.y);
        goal_yaw = static_cast<float> (std::atan2(pose_to_enemy.coeffRef(1), pose_to_enemy.coeffRef(0)));
        auto quaternion = tf::createQuaternionMsgFromYaw(goal_yaw);
        ...
        blackboard_->SetMyToward(escape_goal);
        start = true;
      } else if (!start){
  ...
  bool LoadParam(const std::string &proto_file_path) {
    ...
    obstacle_count_ = decision_config.escape().obstacle_count();
