 ...
 public:
  ...
  void Run() {
    ...
    if (executor_state != BehaviorState::RUNNING 
        || blackboard_->GetDistance(blackboard_->GetRobotMapPose(),
                                    blackboard_->info.my_goal) < blackboard_->threshold.near_dist) {
      ...
      if (sel(generator)==0 && blackboard_->info.shield_buff_active){
         goal = blackboard_->info.my_shield;
      }
      else{
