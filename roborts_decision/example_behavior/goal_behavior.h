...
class GoalBehavior {
 public:
  ...
  void Run() {
    ...
    blackboard_->info.toward_goal.pose.orientation = blackboard_->GetRelativeQuaternion(blackboard_->info.toward_goal,blackboard_->GetRobotMapPose());
    chassis_executor_->SetTowardGoal(blackboard_->info.toward_goal);
  }
