...
class LocalizationNode {
 public:
  ...
  /**
   * @brief 激光扫描消息回调函数(作为定位节点中的主循环)
   * @param laser_scan_msg Laser scan 信息
   */
  void LaserScanCallback(const sensor_msgs::LaserScan::ConstPtr &laser_scan_msg);
  void UwbCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &uwb_info);
  /**
   * @brief 在Rviz手动初始化机器人的init_pose
   * @param init_pose_msg 初始姿态(RViz中的2D姿态估计按钮)
   */
  ...
  /**
   * @brief 发布odom_frame与global_frame坐标系之间的TF变换
   * @return 如果没有errors则为True
   */
  bool PublishTf();
 ...
 private:
  ...
  std::shared_ptr<message_filters::Subscriber<geometry_msgs::PoseWithCovarianceStamped>> uwb_sub_;
  std::unique_ptr<tf::MessageFilter<geometry_msgs::PoseWithCovarianceStamped>> uwb_sub_filter_;
  //ROS Publisher
