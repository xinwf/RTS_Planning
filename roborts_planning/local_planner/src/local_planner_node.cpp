...
void LocalPlannerNode::ExcuteCB(const roborts_msgs::LocalPlannerGoal::ConstPtr &command) {
  ...
  if (plan_mtx_.try_lock()) {
    ...
    toward_goal_.SetPosition(Eigen::Vector2d(command->toward_goal.pose.position.x , command->toward_goal.pose.position.y));
    plan_mtx_.unlock();
...
void LocalPlannerNode::Loop() {
  ...
  while (GetNodeState() == NodeState::RUNNING) {
    ...
    roborts_common::ErrorInfo error_info = local_planner_->ComputeVelocityCommands(cmd_vel_ , toward_goal_);
