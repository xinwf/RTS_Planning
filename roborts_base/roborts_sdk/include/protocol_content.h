 ...
 */
typedef struct {
  int16_t gyro_angle;   //陀螺仪角度
  int16_t gyro_rate;    //陀螺仪角速度,下位机发上来的为角度 x 10的int16类型
  int32_t position_x_mm;  //机器人的x位置,单位mm
  int32_t position_y_mm;  //机器人的y位置,单位mm
  int16_t angle_deg;  //角度,单位/度
  int16_t v_x_mm; //机器人在x方向上的速度,单位:mm/s
  int16_t v_y_mm; //机器人在y方向上的速度,单位:mm/s
} cmd_chassis_info;
