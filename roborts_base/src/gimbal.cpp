...
void Gimbal::ROS_Init(){
  ...
  gimbal_laser_tf_.header.frame_id = "base_link";
  gimbal_laser_tf_.child_frame_id = "gimbal_laser";
}
...
void Gimbal::GimbalInfoCallback(const std::shared_ptr<roborts_sdk::cmd_gimbal_info> gimbal_info){
  ...
  q = tf::createQuaternionMsgFromRollPitchYaw(0.0,
                                              0.0,
                                              gimbal_info->yaw_ecd_angle / 1800.0 * M_PI);
  gimbal_laser_tf_.header.stamp = current_time;
  gimbal_laser_tf_.transform.rotation = q;
  gimbal_laser_tf_.transform.translation.x = 0;
  gimbal_laser_tf_.transform.translation.y = 0;
  gimbal_laser_tf_.transform.translation.z = 0.15;
  tf_broadcaster_.sendTransform(gimbal_laser_tf_);
